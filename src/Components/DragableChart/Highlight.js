...
class Highlight extends AbstractSeries {
  constructor(props){
    ...
    const left= props.start?xScale(props.start)+props.marginLeft:0;
    const right= props.end?xScale(props.end)+props.marginLeft:0;
      this.state = {
        dragging: false,
        brushArea: {top: 0, right, bottom: 0, left},
        brushing: false,
        startLocX: 0,
        startLocY: 0,
        dragArea: null
  ...
  componentWillReceiveProps(props){
    ...
    const left= props.start?xScale(props.start)+props.marginLeft:0;
    const right= props.end?xScale(props.end)+props.marginLeft:0;
    const {top,bottom}=this.state.brushArea;
    this.setState({
        brushArea:{top,bottom,left,right}
    })
  }
    ...
    return (
      ...
      >
        ...
        <rect
          ...
          height={Math.min(Math.max(0, 200), touchHeight)}
        />
      </g>
    );
  }
